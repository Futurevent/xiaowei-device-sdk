 ...
 * QQ 消息的本地持久化存储
 */
public class MsgDbManager implements DbUpgradeListener, DbOpenListener, TableCreateListener {
    ...
    private volatile static MsgDbManager INSTANCE;
    ...
    public void addMsgItem(MsgEntry entry, OnOperationFinishListener listener) {
        if (entry == null) {
    ...
    public void updateMsgItem(MsgEntry entry, OnOperationFinishListener listener) {
        if (entry == null) {
    ...
    public void deleteMsgItems(List<MsgEntry> entries, OnOperationFinishListener listener) {
        if (entries == null) {
    ...
    public void deleteAll() {
        try {
            mDbManager.dropTable(MsgEntry.class);
        } catch (DbException e) {
            e.printStackTrace();
        }
    }
    ...
    private class QueryAllMsgAsyncTask extends AsyncTask<Void, Void, List<MsgEntry>> {
        ...
        @Override
        protected List<MsgEntry> doInBackground(Void... params) {
            if (mDbManager == null) {
            ...
            try {
                return mDbManager.findAll(MsgEntry.class);
            } catch (DbException e) {
                e.printStackTrace();
            }
        ...
        @Override
        protected void onPostExecute(List<MsgEntry> entries) {
    ...
    private class AddMsgAsyncTask extends AsyncTask<MsgEntry, Void, MsgEntry> {
        ...
        @Override
        protected MsgEntry doInBackground(MsgEntry... entries) {
            if (mDbManager == null) {
        ...
        @Override
        protected void onPostExecute(MsgEntry entry) {
    ...
    private class DeleteMsgAsyncTask extends AsyncTask<List<MsgEntry>, Void, MsgEntry> {
        ...
        @Override
        protected MsgEntry doInBackground(List<MsgEntry>... lists) {
            if (mDbManager == null) {
            ...
            for (MsgEntry entry : lists[0]) {
                try {
                    mDbManager.delete(MsgEntry.class, WhereBuilder.b("id", "=", entry.getId()));
                } catch (DbException e) {
                    e.printStackTrace();
                }
            }
