...
public class TriggerAlarmActivity extends BaseActivity {
            ...
            if (focusChange == AudioManager.AUDIOFOCUS_GAIN || focusChange == AudioManager.AUDIOFOCUS_GAIN_TRANSIENT) {
                ...
                if (isFirst) {
                    ...
                    if (!TextUtils.isEmpty(mCurSkillAlarmBean.getEvent())) {
                        XWSDK.getInstance().requestTTS(("提醒 " + mCurSkillAlarmBean.getEvent()).getBytes(), new XWSDK.RequestListener() {
                            @Override
    ...
    private void startPlayRing() {
        ...
        int ret = CommonApplication.mAudioManager.requestAudioFocus(listener, AudioManager.STREAM_MUSIC, AudioManager.AUDIOFOCUS_GAIN);
