 ...
 */
bool Utility::readBufferFromFile(const char *pPath, char *pBuffer, int nInSize, int *pSizeUsed) {
    if (!pPath || !pBuffer) {
        return false;
    }
 ...
 *         message log内容
 */
void Utility::log_func(int level, const char* module, int line, const char* message)
{
    // 建议输出调试级别以上的日志
    if (level < 5) {
        printf("%s\n", message);
